...
export class FirmwareVersionsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.headers = [
      { key: "firmwareVersionNumber", label: "Firmware Version Number", checked: true },
      { key: "assetType", label: "Asset Type", checked: true },
      { key: "model", label: "Asset Model", checked: true },
  ...
  changeDeviceType($event) {
    this.spinner.show();
    let asset = $event.target.value.split(':')[1].trim();
    this.firmForm.controls['assetModel'].enable();
  ...
  deleteRecord(selectedItem) {
    this.spinner.show();
    this.appService.deleteFirmware(`/api/assets/deleteFirmwareVersion/${selectedItem}/${this.selectedAssetFirmId}/${this.selectedFirmwareVersionNumber}/${this.selectedAssetType}/${this.selectedAssetModel}`, '').subscribe(res => {
      if (res.status.success === true) {
        this.reloadDatatable();
