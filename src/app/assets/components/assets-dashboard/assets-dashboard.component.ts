...
export class AssetsDashboardComponent implements OnInit {
  ...
  constructor(
    ...
    forkJoin(
      ...
      this.csStudyDefault = res5.response;
      this.csStudy = res5.response;

      //-- res6  getCustomerSupportIssueWidget ------------------------------
      this.csWidgetDefault = res6.response ? res6.response : [];
      this.csWidget = res6.response ? res6.response : [];
      //new code
      if (res6.response.length > 0) {
        this.totalCount = res6.response[0].totalCount;
        this.Issueoptions.plugins.doughnutlabel.labels[0].text = this.totalCount;
      }
      let dateArr2 = [];
      let weightArr2 = [];
      let issuesCount = [];
      let csMalfunc = res6.response;
      this.asetIsues = res6.response ? res6.response : [];
  ...
  applyFilters4() {
    ...
    this.spinner.show();
    let assetType = filter.assetType ? filter.assetType.issueId : 0;
    let endDate = filter.endDate ? this.customDatePipe.transform(filter.endDate, 'yyyy-MM-dd') : '';
    let startDate = filter.startDate ? this.customDatePipe.transform(filter.startDate, 'yyyy-MM-dd') : '';
    this.assetsService.getAssetsService(`/api/reports/getAssetsDevicesMalfunctions?filterType=datetype&startDate=${startDate}&endDate=${endDate}&filterValue=${assetType}`).subscribe(res => {
      if (res.status.success === true) {
        let dateArr = [];
        let weightArr = [];
    ...
    // }
    // else {
    //   this.toastr.error('Please enter atleast one field to apply!');
    // }
  }
    ...
    this.assetsService.getAssetsService(url).subscribe(res => {
      if (res.status.success === true) {
        ...
        if (res.response && res.response.length > 0) {
          this.totalCount = res.response[0].totalCount;
          this.Issueoptions.plugins.doughnutlabel.labels[0].text = this.totalCount;
        }
        let dateArr = [];
        let weightArr = [];
        let issuesCount = [];
        let csMalfunc = res.response;
        this.asetIsues = res.response ? res.response : [];
        csMalfunc && csMalfunc.forEach((ele, i) => {
