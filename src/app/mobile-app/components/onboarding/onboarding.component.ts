...
export class OnboardingComponent implements OnInit {
  ...
  getHeaders() {
    return [
      ...
      { key: "email", label: "Pet Parent Email ID", checked: true },
      { key: "deviceId", label: "Asset Number", checked: true },
    ...
    if (this.studyForm.valid) {
      ...
      this.mobileAppService.assignStudy(`/api/mobileapp/assignStudyToPet/${petId}/${studyId}`, {}).subscribe(res => {
        if (res.status.success === true) {
          ...
          this.toastr.success('Study has been assigned successfully!');
        }
        else {
