...
export class RolesComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.headers = [
      { key: "roleName", label: "Role Name", clickable: true, checked: true },
      { key: "roleType", label: "Role Type", checked: true },
  ...
  getNode($event) {
    ...
    if ($event.header === 'roleName') {
      this.router.navigate(['/user/roles/view', $event.item.roleId]);
    }
    if (action === 'Edit') {
      this.router.navigate(['/user/roles/edit', $event.item.roleId]);
    }
    if (action === 'Delete') {
  ...
  addEditRoles() {
    this.router.navigate(['/user/roles/add']);
  }
  openPopup(div, size) {
